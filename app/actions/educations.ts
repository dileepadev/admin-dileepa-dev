...
export async function createEducation(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('API Error Response (create education):', errorData);
      const apiMessage =
        (errorData && (errorData.message || errorData.error || JSON.stringify(errorData))) ||
        response.statusText;
      return { message: `Failed to create education: ${apiMessage}` };
    }
...
export async function updateEducation(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error('API Error Response (update education):', errorData);
      const apiMessage =
        (errorData && (errorData.message || errorData.error || JSON.stringify(errorData))) ||
        response.statusText;
      return { message: `Failed to update education: ${apiMessage}` };
    }
